...
# delta_t_range = seq(from = -14, to = 7, by = 7)
### debug 
# delta_t_range = 25:30
nage = length(f_cq)
CI_and_mean = 3
economy_loss_mat = array(0, c(length(delta_t_range), 4, CI_and_mean, 3)) # 4 cities: Wuhan, CQ, BJ, SH
for (scenario_id in 1:3){
  ### if scenario_id == 1, keep original
  if (scenario_id == 2){
      GDP_loss_matrix = rbind(c(daily_cost_SD[1] + daily_cost_SD[1], 0, 0, 0),
                                                c(0, daily_cost_SD[2:4]))
  }
  
  if (scenario_id == 3){
    GDP_loss_matrix  = rbind(c(daily_cost_SD[1], 0, 0, 0),
                                              c(0, daily_cost_SD[2:4]))
  }
  ...
  economy_loss_mat[,,,scenario_id] = economy_loss
  
}
comp_time = Sys.time() - t_init
comp_time

# save(economy_loss, file = "vary_start_date_all_different_q_cost.rda")
save(economy_loss_mat, date, file = "vary_start_date_all_different_q_cost.rda")

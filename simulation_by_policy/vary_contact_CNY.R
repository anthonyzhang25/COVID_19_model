...
t_init = Sys.time()
for (increase in param_range){
  # social_distancing_end = social_distancing_end_vec[ind]
  increase_ind = as.integer(as.character((increase - start)/incre)) + 1
  cat("increase is ",increase, "increase_ind is ", increase_ind, "\n")
  contact_mat = rbind(c(21.09312, 20.44581, 15.02198),# normal
                      c(21.09312, 20.44581, 15.02198) * increase, # cny
                      # c(3, 3, 3))  #  during outbreak
  ...
  # cum_exportion[increase_ind, beta_ind] = sum(WH_output$exported[1:diff_t + 1,,])
  disease_burden[increase_ind, ,] = rowQuantiles(temp_disease_burden, probs = c(0.025, 0.5, 0.975))
  cum_infected[increase_ind, ,] = rowQuantiles(temp_cum_infected, probs = c(0.025, 0.5, 0.975))
  cum_death[increase_ind, ,] = rowQuantiles(temp_cum_death, probs = c(0.025, 0.5, 0.975))
  screening_cost[increase_ind, ,] = rowQuantiles(temp_screening_cost, probs = c(0.025, 0.5, 0.975))
  economy_loss[increase_ind, ,] = rowQuantiles(temp_economy_loss, probs = c(0.025, 0.5, 0.975))
  ### change meadian to mean
  disease_burden[increase_ind, ,2] = rowMeans(temp_disease_burden)
  cum_infected[increase_ind, ,2] = rowMeans(temp_cum_infected)
  cum_death[increase_ind, ,2] = rowMeans(temp_cum_death)
  screening_cost[increase_ind, ,2] = rowMeans(temp_screening_cost)
  economy_loss[increase_ind, ,2] = rowMeans(temp_economy_loss)
  
  # date[increase_ind, 1] = as.Date(base_line_quarantine_startdate + delta_t)
...
save(economy_loss, disease_burden, cum_infected, cum_death, id, param_range, screening_cost, file = "vary_contact_CNY.rda")

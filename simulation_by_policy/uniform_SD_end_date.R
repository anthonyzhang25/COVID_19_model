...
for (delta_t in delta_t_range){
  ...
  for (ind in 1:n_sample){ 
    ...
    SD_end_dates[1] = SD_end_dates[2] = SD_end_dates[3] = as.Date("03/31/20", "%m/%d/%y")
    ...
    SD_end_dates[1] = SD_end_dates[2] = SD_end_dates[3] = social_distancing_end
    time_knot_vec = list('simulation_start' = simulation_start,'simulation_end' = simulation_end, 
                         'chunyun_start' = chunyun_start, 'chunyun_end' = chunyun_end, 
  ...
  date[delta_t_ind] = social_distancing_end
...
save(economy_loss, disease_burden, cum_infected, cum_death, date, delta_t_range, screening_cost, file = "vary_SD_end_date.rda")

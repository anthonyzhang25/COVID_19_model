...
################ sens_policy_3: change THS start time in other cities, and SD in every city
##### SD start on the same date of THS
...
for (delta_t in delta_t_range){
  ...
  for (ind in 1:n_sample){
    ...
    ths_efficiency = 1
    ths_window = 14
    ths_start = quarantine_start + delta_t ###ths: travel history screening
...
save(economy_loss, disease_burden, cum_infected, cum_death, date, delta_t_range, screening_cost, file = "vary_THS_SD_start.rda")

...
##### change SD end time by age group
nage = length(f_cq)
milestone = as.Date("03/31/20", "%m/%d/%y")
SD_end_dates = structure(rep(NA_real_, length(nage)), class="Date")

wuhan_end_dates = structure(rep(NA_real_, length(nage)), class="Date")
wuhan_end_dates[1] = as.Date("03/31/20", "%m/%d/%y")
wuhan_end_dates[2] = as.Date("03/31/20", "%m/%d/%y")
wuhan_end_dates[3] = as.Date("03/31/20", "%m/%d/%y")



id = 2
milestone = as.Date("03/31/20", "%m/%d/%y")
scenario_id_range = 1:5
### debug 
# scenario_id_range = 25:30
nage = length(f_cq)
CI_and_mean = 3
date = structure(rep(NA_real_, length(scenario_id_range)), class="Date")
cum_exportion = array(0, c(length(scenario_id_range), CI_and_mean))
cum_infected = array(0, c(length(scenario_id_range), 4, CI_and_mean)) # 4 cities: Wuhan, CQ, BJ, SH
cum_death = array(0, c(length(scenario_id_range), 4, CI_and_mean)) # 4 cities: Wuhan, CQ, BJ, SH
disease_burden = array(0, c(length(scenario_id_range), 4, CI_and_mean)) # 4 cities: Wuhan, CQ, BJ, SH
economy_loss = array(0, c(length(scenario_id_range), 4, CI_and_mean)) # 4 cities: Wuhan, CQ, BJ, SH
# THS_effectiveness = array(0, c(length(scenario_id_range), 3, CI_and_mean)) # only for CQ, BJ, SH
screening_cost = array(0, c(length(scenario_id_range), 3, CI_and_mean))
SD_end_dates = structure(rep(NA_real_, length(nage)), class="Date")
...
t_init = Sys.time()
for (scenario_id in 1:5){
  ##### scenarios
  # scenario_id = 5
  if (scenario_id == 1){
    SD_end_dates[1] = as.Date("02/29/20", "%m/%d/%y")
    SD_end_dates[2] = as.Date("02/15/20", "%m/%d/%y")
  ...
  if (scenario_id == 3){
    SD_end_dates[1] = as.Date("03/15/20", "%m/%d/%y")
    SD_end_dates[2] = as.Date("02/29/20", "%m/%d/%y")
    SD_end_dates[3] = as.Date("03/31/20", "%m/%d/%y")
  }
  ...
  if (scenario_id == 4){
    SD_end_dates[1] = as.Date("03/15/20", "%m/%d/%y")
    SD_end_dates[2] = as.Date("03/15/20", "%m/%d/%y")
    SD_end_dates[3] = as.Date("03/31/20", "%m/%d/%y")
  }
  ...
  cat( "scenario_id is ", scenario_id, "\n")
  base_line_quarantine_startdate = as.Date("01/23/20", "%m/%d/%y")
  for (ind in 1:n_sample){ # beta = beta_range[2] # 1: min, 2: mean, 3: max
    # if (ind %% 50 == 0){
    #   cat('index is ', ind, "\n")
    # }
    ...
    time_knot_vec = list('simulation_start' = simulation_start,'simulation_end' = simulation_end, 
                         ...
                         'social_distancing_start' = social_distancing_start, 'SD_end_dates' = wuhan_end_dates)
  ...
  # cum_exportion[scenario_id, beta_ind] = sum(WH_output$exported[1:diff_t + 1,,])
  disease_burden[scenario_id, ,] = rowQuantiles(temp_disease_burden, probs = c(0.025, 0.5, 0.975))
  cum_infected[scenario_id, ,] = rowQuantiles(temp_cum_infected, probs = c(0.025, 0.5, 0.975))
  cum_death[scenario_id, ,] = rowQuantiles(temp_cum_death, probs = c(0.025, 0.5, 0.975))
  screening_cost[scenario_id, ,] = rowQuantiles(temp_screening_cost, probs = c(0.025, 0.5, 0.975))
  economy_loss[scenario_id, ,] = rowQuantiles(temp_economy_loss, probs = c(0.025, 0.5, 0.975))
  ### change meadian to mean
  disease_burden[scenario_id, ,2] = rowMeans(temp_disease_burden)
  cum_infected[scenario_id, ,2] = rowMeans(temp_cum_infected)
  cum_death[scenario_id, ,2] = rowMeans(temp_cum_death)
  screening_cost[scenario_id, ,2] = rowMeans(temp_screening_cost)
  economy_loss[scenario_id, ,2] = rowMeans(temp_economy_loss)
  
  # date[scenario_id] = social_distancing_end
  # date[scenario_id, 1] = as.Date(base_line_quarantine_startdate + delta_t)
...
# save(economy_loss, disease_burden, cum_infected, cum_death, date, file = "vary_SD_end_date_by_age.rda")
save(economy_loss, disease_burden, cum_infected, cum_death, date, screening_cost, file = "vary_SD_end_date_by_age.rda")

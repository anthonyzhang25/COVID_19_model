...
t_init = Sys.time()
for (E_I_beta_ratio in param_range){
  # E_I_beta_ratio = ratio
  # ratio_ind = as.integer(ratio/incre) + 1
  ratio_ind = as.integer(as.character((E_I_beta_ratio - start)/incre)) + 1
  cat("E_I_beta_ratio is ",E_I_beta_ratio, "ratio_ind is ", ratio_ind, "\n")
  ...
  # cum_exportion[ratio_ind, beta_ind] = sum(WH_output$exported[1:diff_t + 1,,])
  disease_burden[ratio_ind, ,] = rowQuantiles(temp_disease_burden, probs = c(0.025, 0.5, 0.975))
  cum_infected[ratio_ind, ,] = rowQuantiles(temp_cum_infected, probs = c(0.025, 0.5, 0.975))
  cum_death[ratio_ind, ,] = rowQuantiles(temp_cum_death, probs = c(0.025, 0.5, 0.975))
  screening_cost[ratio_ind, ,] = rowQuantiles(temp_screening_cost, probs = c(0.025, 0.5, 0.975))
  economy_loss[ratio_ind, ,] = rowQuantiles(temp_economy_loss, probs = c(0.025, 0.5, 0.975))
  ### change meadian to mean
  disease_burden[ratio_ind, ,2] = rowMeans(temp_disease_burden)
  cum_infected[ratio_ind, ,2] = rowMeans(temp_cum_infected)
  cum_death[ratio_ind, ,2] = rowMeans(temp_cum_death)
  screening_cost[ratio_ind, ,2] = rowMeans(temp_screening_cost)
  economy_loss[ratio_ind, ,2] = rowMeans(temp_economy_loss)
  
  # date[ratio_ind, 1] = as.Date(base_line_quarantine_startdate + delta_t)
...
save(economy_loss, disease_burden, cum_infected, cum_death, id, param_range, screening_cost, file = "vary_beta_E.rda")

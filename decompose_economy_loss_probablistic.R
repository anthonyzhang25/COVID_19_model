...
death_elderly = array(0, c(length(delta_t_range), 4, CI_and_mean)) # 4 cities: Wuhan, CQ, BJ, SH
...
temp_death_elderly = matrix(0, 4, n_sample)
...
for (ind in 1:n_sample){ 
  ...
  # social_distancing_end = base_line_social_distancing_end + delta_t
  ...
  temp_death_elderly[1, ind] = WH_output$D[diff_t + 1,3]/sum(WH_output$D[diff_t + 1,]) 
  temp_death_elderly[2, ind] = CQ_output$D[diff_t + 1,3]/sum(CQ_output$D[diff_t + 1,]) 
  temp_death_elderly[3, ind] = BJ_output$D[diff_t + 1,3]/sum(BJ_output$D[diff_t + 1,]) 
  temp_death_elderly[4, ind] = SH_output$D[diff_t + 1,3]/sum(SH_output$D[diff_t + 1,]) 
...
death_elderly[delta_t_ind, , ] = rowQuantiles(temp_death_elderly, probs = c(0.025, 0.5, 0.975))
...
death_elderly[delta_t_ind,, 2] = rowMeans(temp_death_elderly)
...
comp_time
save(economy_loss, disease_burden, cum_infected, cum_death, death_elderly, date, file = "status_quo.rda")
...
load("status_quo.rda")
disease_burden[,,2]
round(disease_burden[1,,], 2)  

round(cum_infected[1,,], 2)
round((cum_death[1,,]))
round(economy_loss[1,,], 2)
round(death_elderly[1,,]*100, 2)
